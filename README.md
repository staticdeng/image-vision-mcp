...
通过 [Moonshot](https://platform.moonshot.cn) Kimi 视觉模型（如 `kimi-k2.6`）识别，支持本地文件路径与 http(s):// 图片 URL。
...
┌─────────────────────────────────────────────────────────────────┐
│ Claude Code                                                      │
│   ├─ 调用 MCP 工具 kimi_describe_image → Moonshot API            │
...
编辑 `~/.claude.json`，加入 MCP server 配置，默认支持本地文件 / URL 识图：
...
Claude 会自动调用 `kimi_describe_image` 工具。
